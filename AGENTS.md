...
- `one_word` — server валидирует одно слово (400 если больше), клиент realtime hint
- `reverse_scoring` — swap left/right в transformScore

**Заглушки (`notImplemented: true`):**
- `steal`, `combo` (hooks написаны, флаг просто снять)
- `reverse_round` (нужен pack готовых ответов)
- `blind_round` (нужна grammar-hint БД на вопросы)
- `drunk_mode` (нужен CSS wobble класс на body)
- `leader_abilities` (отдельная фаза `ability_select` + 5 способностей)
...
**Итерация 2 (модификаторы):**
- Включить `steal`, `combo` (hooks готовы, нужно снять `notImplemented` и добавить UI индикаторы streak'ов / визуала кражи)
- `drunk_mode` — CSS class на body с wobble/floating animations
