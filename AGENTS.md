...
- ✅ **Итерация 5 (host screen redesign) & Польские улучшения**:
  - **Театральный занавес (Curtain Transition)**: Створки из красного бархата с эффектом 3D складок и теней. Автоматический перехват смены фазы в snapshot-дистрибьюторе (закрыть -> сменить DOM -> открыть), исключая мелкие переходы `vote` <-> `vote_result`.
  - **Деревянная сцена и прожекторы**: Сцена в перспективе (`wooden-stage.png`) и прожекторы Radial-Gradient с фокусом на подиуме лидера (1-е место) после 1.5с ожидания.
  - **Анонимный режим**: Полное скрытие блоков авторов (кругов-аватарок и имен) на хост-экране в фазе голосования.
  - **Анимация дуэлей (Fly-out / Fly-in)**: Карты и вопросы улетают влево при смене дуэли и прилетают справа. Эффекты `winner-pop` (золотое свечение, пульсация) и `loser-shrink` (grayscale, уменьшение, наклон) при раскрытии результатов.
  - **Живые реакции (Live Reactions)**: Панель кнопок-эмодзи (😂, 💩, 🔥, 🤔, 🤡) на мобильном экране игроков, POST-эндпоинт на сервере и SSE-трансляция с летящими снизу вверх эмодзи на хосте.
  - **Редактор наборов**: Восстановлены кнопки сохранения, экспорта/импорта JSON наборов вопросов, а также добавление мемов.
  - **Безопасная загрузка мемов**: Загрузка картинок с устройства напрямую с автоматическим клиентским Canvas-сжатием в JPEG (макс. 1200px, 85% качество) и серверной JPEG-валидацией заголовка.
...
- 🔄 **Игровой режим / Модификатор «Ассоциации»**:
  - Добавить новый игровой режим или модификатор, где игрокам нужно вводить ассоциации к словам (например, к предложенным терминам или концептам).
  - Настроить визуализацию и валидацию ответов для ассоциаций.
- 🔄 **Оптимизация и тестирование**:
  - Дополнительное тестирование производительности и плавности CSS-анимаций (особенно покачивания прожекторов и летящих эмодзи) на смартфонах и слабых хост-экранах (например, на ТВ-браузерах).
